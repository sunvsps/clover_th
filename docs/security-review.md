...
> **Status update (2026-09-25):** commit `edd588f` ("Fix security review findings (H-1..H-3, M-1..M-6 and safe Lows)") landed on `dev` and fixes all 3 High and all 6 Medium findings below, plus the Low findings marked **RESOLVED** in the table. Findings still marked **OPEN** were not touched by that commit and have not been re-verified since. This file is otherwise left as the original point-in-time report (scope: WP1-WP10 at HEAD `1939639`) — read the individual entries in that light.
...
> Status as of `edd588f` (2026-09-25): items 1-7 are done. Item 8 is partial (L-1, L-3, L-4 done; L-2 only the `permissions:` block, SHA-pinning still open; L-11 untouched). Items 9-10 are unverified since the original review — see per-finding status in the tables above.

1. ~~H-1 + H-2: bot guard verifies the key first; `trustProxy` by hop count; document the proxy header; add tests.~~ **DONE**
2. ~~H-3: isolate `lazyFinalize` failures (log, keep serving reads).~~ **DONE**
3. ~~M-1: move the rate limiter ahead of the session lookup (or add a pre-auth IP counter).~~ **DONE**
4. ~~M-2: complete the polling fingerprint (name, cap, timing, item content).~~ **DONE**
5. ~~M-3: category locks in deactivation; filter inactive members in the allocation snapshot.~~ **DONE**
6. ~~M-5 + M-4: session purge, absolute session lifetime, login/logout audit; reject placeholder `SESSION_SECRET` in production.~~ **DONE**
7. ~~M-6 and L-5: redirect state failures to the frontend~~ **DONE**; hide draft rounds on write paths (L-5) — **still OPEN**.
8. Ops/CI hygiene: L-1, L-3, L-4 **DONE**; L-2 **PARTIAL** (permissions block added, actions still tag-pinned not SHA-pinned); L-11 (`node dist` start) — **still OPEN**; I-5 (loopback bind) not re-verified.
9. Admin workflow gaps: item release/reassign, window extend, revoke-admin, admin health counts — **still OPEN** (`backend/scripts/` has no `revoke-admin.ts` as of this audit).
10. Quality clean-ups: audit `meta` typing, duplicate schemas, batch inserts, missing tests (L-12) — **still OPEN**; `FOR UPDATE` on member PATCH (L-9) is **DONE**.
