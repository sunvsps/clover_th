...
| `LOCAL_DEMO_ENABLED` | no (false) | **LOCAL DEMOS ONLY. NEVER SET THIS IN PRODUCTION.** `true` registers `/api/v1/demo/*`, a login without Discord that lists members and signs in as any of them (admins included) for callers on the same machine. Only the strings `true` and `false` are accepted. **The server refuses to start with `LOCAL_DEMO_ENABLED=true` and `NODE_ENV=production`**, and (like `dev-login`) with a non-local `DATABASE_URL` or an https `FRONTEND_URL` unless `DEV_LOGIN_ALLOW_REMOTE_DB=1`. With it off the routes do not exist and are not in the OpenAPI document |
