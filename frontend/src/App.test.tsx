...
/** Signs in as the demo admin and runs a round (3 s countdown), leaving the round open. */
...
describe("App shell", () => {
  it("renders the top bar, the three feature tabs and the auction page", () => {
    render(<App />);
  ...
  it("reserving before signing in only shows a hint: the buttons are disabled", () => {
    render(<App />);
    expect(screen.getAllByRole("button", { name: /Reserve/ })[0]).toBeDisabled();
  });
});
...
describe("App: reservation flow (behaviour kept from before the extraction)", () => {
  it("reserve, summary, received marks, remove", async () => {
    ...
    expect(screen.getByText("Page 1 / Item 1 is reserved for Mew.")).toBeInTheDocument();
    expect(within(first).getByText("Reserved by Mew")).toBeInTheDocument();
    ...
    expect(within(summary).getByText("Mew")).toBeInTheDocument();
  ...
  it("signing out disables reserving again; the toast can be dismissed", async () => {
    vi.useFakeTimers();
    const user = userEvent.setup({ delay: null });
    render(<App />);
    await openRound(user);
    ...
    expect(within(screen.getByLabelText("Page 4")).getAllByRole("button", { name: /Reserve/ })[0]).toBeDisabled();
  });
});
